(ns orbit.core
  ...
  (:require [orbit.extension :as extension]
...
(defn full-orbit-parallel
  [seeds sa]
  (full-orbit seeds sa extension/parallel-step))
...
(defn full-orbit-bulk
  ...
  [seeds sa]
  (full-orbit seeds sa extension/bulk-step))
...
(defn full-orbit-single
  ...
  [seeds sa]
  (full-orbit seeds sa extension/single-step))
...
(defn first-solution-single
  ...
  (first-solution seed sa candidate? solution? extension/single-step))

(defn first-solution-bulk
  "Returns a first solution when searching by depth-first."
  [seed sa candidate? solution?]
  (first-solution seed sa candidate? solution? extension/bulk-step))
...
(defn acyclic-search-bulk
  [seeds sa solution?]
  (acyclic-search seeds sa solution? extension/bulk-step))
